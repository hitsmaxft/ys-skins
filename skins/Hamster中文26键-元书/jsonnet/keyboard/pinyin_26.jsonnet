...
local createButton(params={}) =
  ...
  std.prune({
    ...
    [if isLetter then 'notification']: [params.key + 'FlypyYunmuNotification', params.key + 'FlypySchemaReadyNotification'],
...
local keyboard(theme, orientation) =
  {
    ...
    toolbarHeight: if theme == 'dark' then 25 else others[if orientation == 'portrait' then '竖屏' else '横屏']['toolbar高度'],
    ...
    commaButtonForegroundStyle2: utils.makeTextStyle(
      params = {
        text: '。',
        normalColor: color[theme]['按键前景颜色'],
        highlightColor: color[theme]['按键前景颜色'],
...

{
  new(theme, orientation):
    ...
    { [k + 'FlypySchemaReadyNotification']: {
        notificationType: 'rime', rimeNotificationType: 'schemaChanged',
